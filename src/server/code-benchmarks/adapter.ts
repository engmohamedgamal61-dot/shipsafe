 ...
 * `scorer.ts`'s normalized `ReviewerResult`, for the Code Reviewer.
 * Identical in design and behavior to `security-benchmarks/adapter.ts`
 * (same persisted shape, same field-by-field mapping, same "never
 * guess" refusal policy) — kept as an independent copy per
 * `tests/code-benchmarks/README.md`, since the two benchmarks' scoring
 * must stay logically separate even though the underlying persisted
 * `Finding` type is shared production infrastructure.
 */
