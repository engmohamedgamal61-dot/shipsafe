...
export const COMPATIBILITY_BASELINE_NOTE = "This is a compatibility baseline of the CURRENT production Code Reviewer, measured as-shipped.";
...
export function renderBaselineMarkdown(report: BaselineReport): string {
  const lines: string[] = [];
  lines.push("# Code Reviewer — Current Production Compatibility Baseline");
  lines.push("");
  ...
  lines.push(`| Code benchmark schema version | \`${report.metadata.versions.codeBenchmarkSchemaVersion}\` |`);
  lines.push(`| Code benchmark dataset version | \`${report.metadata.versions.codeBenchmarkDatasetVersion}\` |`);
  lines.push(`| Code reviewer prompt version | \`${report.metadata.versions.codeReviewerPromptVersion}\` |`);
  ...
  lines.push("- The unmodified production `CodeReviewerAgent` + `AnthropicProvider` classes are invoked directly — this harness never re-implements reviewer logic.");
  lines.push("- `ReviewOrchestrator`'s other four specialist reviewers and Release Judge are NOT invoked — only the Code Reviewer is graded.");
...
export function writeBaselineArtifacts(report: BaselineReport, repoRoot: string): { jsonPath: string; markdownPath: string } {
  const jsonPath = path.join(repoRoot, "artifacts", "code-benchmark", "current-baseline.json");
  const markdownPath = path.join(repoRoot, "docs", "agents", "code-baseline-current.md");
