...
/**
 * Zod validation for the Code Reviewer benchmark's fixture manifest
 * shape — the ground-truth contract for `tests/code-benchmarks/`.
 *
 * Deliberately parallel to, but INDEPENDENT of,
 * `src/server/security-benchmarks/schema.ts`: same architecture
 ...
 * see `tests/code-benchmarks/README.md` for why the two benchmarks are
 * kept apart rather than sharing one scorer. Never imported by
 * `src/server/review-engine/` or by `src/server/security-benchmarks/`.
 */
...
export const tagSchema = z.enum(["buggy", "safe", "ambiguous", "false_positive_trap", "multi_file", "concurrency"]);
 ...
 * kebab-case (e.g. `correctness.off-by-one`, `performance.algorithmic-complexity`).
 * Enforced on the GROUND-TRUTH side only (`allowed_categories`,
 * `prohibited_categories`, ground-truth `category`/`alternate_categories`)
 * — never on a produced finding's own category, which today's
 * production Code Reviewer emits as free-form text (mirrors the
 * security benchmark's own `categorySchema` design for the identical
 * reason: production must be benchmarkable unmodified).
 */
export const categorySchema = z
  .string()
  .regex(
    /^[a-z0-9]+(?:-[a-z0-9]+)*\.[a-z0-9]+(?:-[a-z0-9]+)*$/,
    'category must follow the canonical "domain.subcategory" format (e.g. "correctness.off-by-one")',
  );

/** `CODE-<DOMAIN>-NNN` — a stable benchmark identity for a root cause, independent of category/location. Not currently used as a primary match key (production emits no rule id), kept for parity with the security benchmark's schema and for a future reviewer that does emit one. */
export const ruleIdSchema = z
  .string()
  .regex(/^CODE-[A-Z]+-\d{3}$/, 'rule_id must follow the format "CODE-<DOMAIN>-NNN" (e.g. "CODE-CORRECTNESS-001")');
...
export const groundTruthFindingSchema = z.object({
  ...
  /** Human-readable ground truth for WHY this is a real, confirmable defect (or, on an optional/ambiguous entry, what specifically remains unconfirmed) — the code-review analogue of the security benchmark's `exploit_preconditions`, renamed since "exploit" doesn't apply outside security findings. Not scored directly; used when hand-auditing a run. */
  ...
  /** Zero or more ADDITIONAL canonical categories that also validly name this same root cause — see `security-benchmarks/schema.ts`'s identically-named field for the full design rationale (explicit, hand-authored, cross-checked for overlap-safety below). */
