...
function persistedFinding(overrides: Partial<PersistedFinding> = {}): PersistedFinding {
  return {
    ...
    description: "items.slice(start, end + 1) returns pageSize + 1 items.",
    filePath: "paginate.ts",
    lineStart: 6,
    lineEnd: 6,
    category: "correctness",
    recommendation: "Use items.slice(start, end).",
    confidence: 0.9,
    ...overrides,
  };
}
...
describe("adaptPersistedFindings", () => {
  ...
  it("preserves file, line, category, and maps description -> evidence", () => {
    const result = adaptPersistedFindings([persistedFinding({ filePath: "src/foo.ts", lineStart: 3, lineEnd: 9, category: "performance", description: "quoted code" })]);
    const [finding] = result.findings;
    expect(finding).toMatchObject({ file: "src/foo.ts", lineStart: 3, lineEnd: 9, category: "performance", evidence: "quoted code" });
  });
