...
function finding(overrides: Partial<ProducedFinding> = {}): ProducedFinding {
  return producedFindingSchema.parse({
    ...
    lineStart: 1,
    lineEnd: 5,
    evidence: "computeAutoMergeEligibility",
    ...overrides,
  });
}
...
describe("buildBaselineReport", () => {
  ...
  it("builds a full fixture report for a scored, passing fixture", () => {
    const run = scoredRun(missingTests, [finding()]);
    const report = buildBaselineReport([run], metadata(), fixturesById([missingTests]));
    ...
    expect(fixtureReport.findings[0]?.evidence).toBe("computeAutoMergeEligibility");
  });

  it("records a provider_failure fixture without a score", () => {
    const run: BaselineFixtureRun = {
      fixtureId: missingTests.manifest.fixture_id,
      domain: "missing-tests",
      tags: ["buggy"],
      fixtureDir: missingTests.fixtureDir,
  ...
  it("aggregates safe-code false-positive rate via extraMetrics", () => {
    const safeDirty = scoredRun(safeSuite, [finding({ category: "missing-error-path.generic", file: "score.test.ts", lineStart: 5, lineEnd: 5 })]);
    const report = buildBaselineReport([safeDirty], metadata(), fixturesById([safeSuite]));
...
describe("renderBaselineMarkdown", () => {
  ...
  it("renders a per-fixture table row and category-accuracy metric", () => {
    const run = scoredRun(missingTests, [finding()]);
    const markdown = renderBaselineMarkdown(buildBaselineReport([run], metadata(), fixturesById([missingTests])));
    expect(markdown).toContain(missingTests.manifest.fixture_id);
...
describe("writeBaselineArtifacts", () => {
  ...
  it("writes both a JSON report and a markdown report to the expected relative paths", () => {
    dir = mkdtempSync(path.join(tmpdir(), "test-reviewer-benchmark-test-"));
    const run = scoredRun(missingTests, [finding()]);
    const report = buildBaselineReport([run], metadata(), fixturesById([missingTests]));
    const { jsonPath, markdownPath } = writeBaselineArtifacts(report, dir);

    expect(jsonPath).toBe(path.join(dir, "artifacts", "test-reviewer-benchmark", "current-baseline.json"));
    expect(markdownPath).toBe(path.join(dir, "docs", "agents", "test-reviewer-baseline-current.md"));
