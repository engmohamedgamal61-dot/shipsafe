 ...
 * Anthropic provider for the Test Reviewer benchmark — every call here
 ...
 * plain `npm test` (mirrors the other four benchmarks'
 * `baseline/run.live.test.ts`).
 *
 * Run explicitly with:
 *   RUN_LIVE_TEST_REVIEWER_BASELINE=1 npx vitest run src/server/test-reviewer-benchmarks/baseline/run.live.test.ts
 */
describe.skipIf(!process.env.RUN_LIVE_TEST_REVIEWER_BASELINE)("LIVE test-reviewer benchmark baseline (real Anthropic calls)", () => {
  it(
    "runs the current production Test Reviewer against all 10 fixtures and writes the baseline artifacts",
    async () => {
