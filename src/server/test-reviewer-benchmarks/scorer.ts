...
/**
 * Deterministic Test Reviewer benchmark scorer. Architecturally mirrors
 * `code-benchmarks/scorer.ts`, `security-benchmarks/scorer.ts`,
 * `database-benchmarks/scorer.ts`, and `architecture-benchmarks/scorer.ts`
 * (same classification model, same fabrication-detection design —
 * carrying forward the FIXED evidence-verification logic, including the
 * leading-dot/empty-parens method-reference exemption, per "do not
 * weaken evidence verification") — but an independent module, scoring
 * an independent category taxonomy. Never imported by
 * `src/server/review-engine/` or by any other benchmark.
 */
...
// ---------------------------------------------------------------------------
// Fabricated-evidence detection — same design as the other four benchmarks' scorers
// ---------------------------------------------------------------------------
 ...
 * optionally carry its own empty call parens (`select().in()`,
 * `from().select().in()`, `.filter()`) — confirmed necessary by this
 * benchmark's own first live baseline run, where a finding described a
 * mock's chained method shape (`` `select().in()` ``) to refer to what
 * the fixture actually implements as nested object-literal properties.
 * Broadened from a single trailing `()` (which only allowed one final
 * call) to a call after ANY segment, since describing a multi-step
 * method chain this way is standard, common prose shorthand, not a
 * claim that literal chain syntax appears in the source.
 */
const DOTTED_IDENTIFIER_PATTERN = /^\.?[A-Za-z_$][\w$]*(?:\(\))?(?:\.[A-Za-z_$][\w$]*(?:\(\))?)*$/;

/**
 * A bare reference to a source FILE by a plausible name and common
 * extension (`` `score.ts` ``, `` `sign-up-validation.ts` ``) — confirmed
 * necessary by this benchmark's own first live baseline run, where a
 * finding accurately named the fixture's OWN file, or a sibling
 * production file genuinely not included in the diff, and was wrongly
 * flagged as fabricated because a file's own path is never part of its
 * CONTENT (`sourceText` here is file contents, not the diff's `+++ b/...`
 * header). Naming a plausible file is categorically different from
 * quoting a fabricated code statement or value — there is no "wrong
 * quote" a bare filename mention can make in the way a fake function
 * call or made-up return value can, so exempting this narrow shape does
 * not weaken genuine fabrication detection for actual code claims.
 */
const FILE_REFERENCE_SHAPE = /^[\w-]+\.(?:ts|tsx|js|jsx|mjs|cjs|json|sql|md)$/i;

/**
 * Common JavaScript/TypeScript reserved words and literals, referenced
 * BARE (the entire backtick span, nothing else) — confirmed necessary
 * by this benchmark's own first live baseline run, where the model
 * discussed a missing `await`/`return` or a function's `true`/`false`
 * return value using the same backtick styling as an actual source
 * quote. This is universal, standard technical-writing convention for
 * naming a language construct, not a claim that the literal token
 * appears in this specific file. Deliberately a small, closed,
 * universal set — not project-specific identifiers — and only exempts
 * an EXACT, single-word match, so a multi-word span using one of these
 * words as part of a larger, still-checkable claim is unaffected.
 */
const COMMON_LANGUAGE_LITERALS = new Set(["true", "false", "null", "undefined", "nan", "await", "async", "return", "throw", "void", "try", "catch", "finally"]);
...
/**
 * Treats `'` and `"` as the same character — JS/TS string literals mean
 * the same thing regardless of which quote style encloses them, so a
 * finding describing a double-quoted source string with single quotes
 * (or vice versa) is not misquoting its content, only its punctuation
 * style. Confirmed necessary by this benchmark's own first live
 * baseline run, where a finding accurately described a mock's
 * single-element array using single quotes while the source used double
 * quotes.
 */
...
function isVerifiableApiReference(span: string, sourceText: string): boolean {
  ...
  return segments.length > 0 && segments.every((segment) => new RegExp(`\\b${escapeForRegExp(segment)}\\b`).test(sourceText));
}

function isFileReferenceMention(span: string): boolean {
  return FILE_REFERENCE_SHAPE.test(span);
}

function isCommonLanguageLiteral(span: string): boolean {
  return COMMON_LANGUAGE_LITERALS.has(span.toLowerCase());
}
...
function hasFabricatedCodeClaim(finding: ProducedFinding, sourceText: string): boolean {
  ...
  const normalizedSource = normalizeQuoteStyle(collapseWhitespace(sourceText).toLowerCase());
  return spans.some(({ span, precedingContext }) => {
    const candidate = stripTrailingPunctuation(span);
    if (candidate.length === 0) return false;
    if (normalizedSource.includes(normalizeQuoteStyle(candidate.toLowerCase()))) return false;
    if (isIllustrativeExample(precedingContext)) return false;
    if (isVerifiableApiReference(candidate, sourceText)) return false;
    if (isFileReferenceMention(candidate)) return false;
    if (isCommonLanguageLiteral(candidate)) return false;
    return true;
  });
}
