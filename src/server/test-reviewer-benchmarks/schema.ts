...
/**
 * Zod validation for the Test Reviewer benchmark's fixture manifest
 * shape — the ground-truth contract for
 * `tests/test-reviewer-benchmarks/`.
 *
 * Deliberately parallel to, but INDEPENDENT of,
 * `src/server/code-benchmarks/schema.ts`, `security-benchmarks/schema.ts`,
 * `database-benchmarks/schema.ts`, and `architecture-benchmarks/schema.ts`:
 * same architecture (required/optional/prohibited findings, alternate
 * categories, severity/confidence ranges, ambiguous-fixture handling)
 * because that architecture is domain-agnostic, but a completely
 * separate module — see `tests/test-reviewer-benchmarks/README.md` for
 * why the five benchmarks are kept apart rather than sharing one
 * scorer. Never imported by `src/server/review-engine/` or by any other
 * benchmark.
 */
 ...
 * kebab-case (e.g. `weak-assertions.tautological`,
 * `async-mistakes.missing-await`). Enforced on the GROUND-TRUTH side
 * only — never on a produced finding's own category, which today's
 * production Test Reviewer emits as free-form text (mirrors the other
 * four benchmarks' identically-purposed `categorySchema`, for the
 * identical reason: production must be benchmarkable unmodified).
 */
export const categorySchema = z
  .string()
  .regex(
    /^[a-z0-9]+(?:-[a-z0-9]+)*\.[a-z0-9]+(?:-[a-z0-9]+)*$/,
    'category must follow the canonical "domain.subcategory" format (e.g. "weak-assertions.tautological")',
  );

/** `TEST-<DOMAIN>-NNN` — a stable benchmark identity for a root cause, independent of category/location. Not currently used as a primary match key (production emits no rule id), kept for parity with the other benchmarks' schemas and for a future reviewer that does emit one. */
export const ruleIdSchema = z
  .string()
  .regex(/^TEST-[A-Z]+-\d{3}$/, 'rule_id must follow the format "TEST-<DOMAIN>-NNN" (e.g. "TEST-WEAKASSERTIONS-001")');
...
export const groundTruthFindingSchema = z.object({
  ...
  /** Human-readable ground truth for WHY this is a real, confirmable test-quality defect (or, on an optional/ambiguous entry, what specifically remains unconfirmed). Not scored directly; used when hand-auditing a run. */
