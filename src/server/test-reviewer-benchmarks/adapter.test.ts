...
function persistedFinding(overrides: Partial<PersistedFinding> = {}): PersistedFinding {
  return {
    ...
    description: "The test only asserts toBeDefined(), which can never fail.",
    filePath: "verdict.test.ts",
    lineStart: 5,
    lineEnd: 8,
    category: "weak-assertions",
    recommendation: "Assert the actual computed value.",
    confidence: 0.9,
    ...overrides,
  };
}
...
describe("adaptPersistedFindings", () => {
  ...
  it("preserves file, line, category, and maps description -> evidence", () => {
    const result = adaptPersistedFindings([persistedFinding({ filePath: "src/foo.test.ts", lineStart: 3, lineEnd: 9, category: "async-mistakes", description: "quoted code" })]);
    const [finding] = result.findings;
    expect(finding).toMatchObject({ file: "src/foo.test.ts", lineStart: 3, lineEnd: 9, category: "async-mistakes", evidence: "quoted code" });
  });
  ...
  it("adapts multiple valid findings in order", () => {
    const result = adaptPersistedFindings([persistedFinding({ id: "a", filePath: "a.test.ts" }), persistedFinding({ id: "b", filePath: "b.test.ts" })]);
    expect(result.findings.map((f) => f.file)).toEqual(["a.test.ts", "b.test.ts"]);
  });
});
