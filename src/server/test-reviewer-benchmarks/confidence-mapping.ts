...
/**
 * The Test Reviewer benchmark's own canonical numeric-confidence
 * bucketing — identical thresholds to, but a separate copy from, the
 * other four benchmarks' `confidence-mapping.ts` (kept independent per
 * `tests/test-reviewer-benchmarks/README.md`; all five derive from the
 * same persisted `Finding.confidence` domain field, so it would be
