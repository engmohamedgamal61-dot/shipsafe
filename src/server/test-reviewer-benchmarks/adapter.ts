 ...
 * `scorer.ts`'s normalized `ReviewerResult`, for the Test Reviewer.
 * Identical in design and behavior to the other four benchmarks'
 * `adapter.ts` (same persisted shape, same field-by-field mapping, same
 * "never guess" refusal policy) — kept as an independent copy per
 * `tests/test-reviewer-benchmarks/README.md`, since all five
