 ...
 * `scorer.ts`'s normalized `ReviewerResult`, for the Database Reviewer.
 * Identical in design and behavior to `code-benchmarks/adapter.ts` and
 * `security-benchmarks/adapter.ts` (same persisted shape, same
 * field-by-field mapping, same "never guess" refusal policy) — kept as
 * an independent copy per `tests/database-benchmarks/README.md`, since
 * all three benchmarks' scoring must stay logically separate even though
 * the underlying persisted `Finding` type is shared production
 * infrastructure.
 */
