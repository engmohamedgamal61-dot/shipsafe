...
/**
 * Zod validation for the Database Reviewer benchmark's fixture manifest
 * shape — the ground-truth contract for `tests/database-benchmarks/`.
 *
 * Deliberately parallel to, but INDEPENDENT of,
 * `src/server/code-benchmarks/schema.ts` and
 ...
 * see `tests/database-benchmarks/README.md` for why the three
 * benchmarks are kept apart rather than sharing one scorer. Never
 * imported by `src/server/review-engine/`, `code-benchmarks/`, or
 * `security-benchmarks/`.
 */
 ...
 * kebab-case (e.g. `foreign-keys.missing-reference`,
 * `race-condition.missing-unique-constraint`). Enforced on the
 * GROUND-TRUTH side only — never on a produced finding's own category,
 * which today's production Database Reviewer emits as free-form text
 * (mirrors the code/security benchmarks' identically-purposed
...
export const categorySchema = z
  ...
  .regex(
    ...
    'category must follow the canonical "domain.subcategory" format (e.g. "foreign-keys.missing-reference")',
  );

/** `DB-<DOMAIN>-NNN` — a stable benchmark identity for a root cause, independent of category/location. Not currently used as a primary match key (production emits no rule id), kept for parity with the other two benchmarks' schemas and for a future reviewer that does emit one. */
export const ruleIdSchema = z
  .string()
  .regex(/^DB-[A-Z]+-\d{3}$/, 'rule_id must follow the format "DB-<DOMAIN>-NNN" (e.g. "DB-FOREIGNKEYS-001")');
...
export const groundTruthFindingSchema = z.object({
  ...
  /** Human-readable ground truth for WHY this is a real, confirmable defect (or, on an optional/ambiguous entry, what specifically remains unconfirmed). Not scored directly; used when hand-auditing a run. */
