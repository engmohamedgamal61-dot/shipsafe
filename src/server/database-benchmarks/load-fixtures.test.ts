...
describe("discoverFixtureDirs / validateAllFixtures (real fixtures)", () => {
  ...
  it("loadFixture reads every declared file's source text", () => {
    const dir = path.join(BENCHMARK_ROOT, "foreign-keys", "01-missing-foreign-key-reference");
    const fixture = loadFixture(dir);
    expect(fixture.sourceFiles["migration.sql"]).toContain("repository_id uuid not null,");
  });
});
...
describe("validateFixture (synthetic error cases)", () => {
  ...
  it("reports invalid JSON without throwing", () => {
    dir = mkdtempSync(path.join(tmpdir(), "db-bench-test-"));
