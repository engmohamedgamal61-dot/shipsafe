...
/**
 * Deterministic Database Reviewer benchmark scorer. Architecturally
 * mirrors `code-benchmarks/scorer.ts` and `security-benchmarks/scorer.ts`
 ...
 * leading-dot/empty-parens method-reference exemption confirmed
 * necessary by the Code Reviewer benchmark's own first live run, per "do
 * not weaken evidence verification") — but an independent module,
 * scoring an independent category taxonomy. Never imported by
 * `src/server/review-engine/`, `code-benchmarks/`, or
 * `security-benchmarks/`.
 *
 * Deliberately DOES NOT carry over the security benchmark's
 * `evidenceState`/`insufficient_evidence` machinery — not part of this
 * benchmark's requested metric set.
 */
...
// ---------------------------------------------------------------------------
// Fabricated-evidence detection — same design as the other two benchmarks' scorers
// ---------------------------------------------------------------------------

const CODE_QUOTE_PATTERN = /`([^`]+)`/g;
const ILLUSTRATIVE_EXAMPLE_MARKER = /\b(?:e\.g\.,?|for example,?|such as|like)\s*$/i;
/**
 * A generic API/method/column reference, not a literal invocation:
 * - a dotted chain (`workspace_memberships.workspace_id`), optionally with trailing `()`,
 * - OR a single leading-dot method reference (`.eq()`, `.in()`) — the
 *   shorthand for "the X call" without naming a receiver, confirmed as a
 *   real gap by the Code Reviewer benchmark's own first live run.
 *
 * Deliberately requires EMPTY parens only (`\(\)`) — content inside
 * parens (real or fabricated arguments) is never covered by this
 * pattern, so a span like `` `references public.workspaces (id)` `` still
 ...
 * fabrication. See `code-benchmarks/scorer.ts`'s identically-shaped
 ...
 * evidence-verification logic, not anything code- or security-specific.
 */
...
export interface SuiteMetrics {
  ...
  /** Of matched_required findings, the fraction whose category (raw or normalized) equals one of the entry's own expected category set — vacuously 1 when no matched_required findings exist. */
