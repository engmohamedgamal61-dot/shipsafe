 ...
 * Anthropic provider for the Database Reviewer benchmark — every call
 * here costs real money and hits a real external API. Guarded behind an
 * explicit env var so it is SKIPPED by default, including during a
 * plain `npm test` (mirrors `code-benchmarks/baseline/run.live.test.ts`).
 *
 * Run explicitly with:
 *   RUN_LIVE_DATABASE_BASELINE=1 npx vitest run src/server/database-benchmarks/baseline/run.live.test.ts
 */
describe.skipIf(!process.env.RUN_LIVE_DATABASE_BASELINE)("LIVE database benchmark baseline (real Anthropic calls)", () => {
  it(
    "runs the current production Database Reviewer against all 10 fixtures and writes the baseline artifacts",
    async () => {
