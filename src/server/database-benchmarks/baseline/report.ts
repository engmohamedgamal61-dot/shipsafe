...
export const COMPATIBILITY_BASELINE_NOTE = "This is a compatibility baseline of the CURRENT production Database Reviewer, measured as-shipped.";
...
export function renderBaselineMarkdown(report: BaselineReport): string {
  const lines: string[] = [];
  lines.push("# Database Reviewer — Current Production Compatibility Baseline");
  lines.push("");
  ...
  lines.push(`| Database benchmark schema version | \`${report.metadata.versions.databaseBenchmarkSchemaVersion}\` |`);
  lines.push(`| Database benchmark dataset version | \`${report.metadata.versions.databaseBenchmarkDatasetVersion}\` |`);
  lines.push(`| Database reviewer prompt version | \`${report.metadata.versions.databaseReviewerPromptVersion}\` |`);
  ...
  lines.push("- The unmodified production `DatabaseReviewerAgent` + `AnthropicProvider` classes are invoked directly — this harness never re-implements reviewer logic.");
  lines.push("- `ReviewOrchestrator`'s other four specialist reviewers and Release Judge are NOT invoked — only the Database Reviewer is graded.");
...
export function writeBaselineArtifacts(report: BaselineReport, repoRoot: string): { jsonPath: string; markdownPath: string } {
  const jsonPath = path.join(repoRoot, "artifacts", "database-benchmark", "current-baseline.json");
  const markdownPath = path.join(repoRoot, "docs", "agents", "database-baseline-current.md");
