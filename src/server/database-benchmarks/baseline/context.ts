 ...
 * specialist reviewer, from a Database Reviewer benchmark fixture's full
 * source files. Identical in design to `code-benchmarks/baseline/context.ts`
 * (see that file's own doc comment for the full rationale on the
 * synthetic "new file" diff construction and why its line numbers
 * exactly match the fixture file's own) — kept as an independent copy
 * per `tests/database-benchmarks/README.md`.
 */
export function buildReviewContextFromFixture(fixture: LoadedFixture): ReviewContext {
  ...
  return {
    pullRequestTitle: `[database-benchmark] ${fixture.manifest.fixture_id}`,
