...
import { DatabaseReviewerAgent } from "@/server/review-engine/agents/database-reviewer";
...
/**
 * Invokes the CURRENT, unmodified production `DatabaseReviewerAgent` +
 * `AnthropicProvider` — identical in design to
 * `code-benchmarks/baseline/run.ts` (see that file's own doc comment for
 * why `ReviewOrchestrator` itself is deliberately NOT reused: it would
 * run all five specialist reviewers plus the Release Judge for output
 * this benchmark doesn't grade). Never imported from `npm test`'s
 * default run — the only entry point is `run.live.test.ts`, gated
 * behind an explicit env var, since every call here costs real money and
 * hits a real external API.
 */
const MAX_ATTEMPTS = 2;

async function reviewWithRetry(
  agent: DatabaseReviewerAgent,
...
export async function runFixtureLive(fixture: LoadedFixture, agent: DatabaseReviewerAgent): Promise<BaselineFixtureRun> {
